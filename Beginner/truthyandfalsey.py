...
# The above example will first check if is_citizen is True. If so, it will then go to the nested if statement and check if age is greater than or equal to 18.
...
# Many values are considered truthy, that is, they evaluate to True in a logical context.
# Others are falsy, meaning they evaluate to False.
...
# The and operator takes two operands and returns the first operand if it is falsy, otherwise, it returns the second operand.
# Both operands must be truthy for an expression to result in a truthy value.
# Example Code
is_citizen = True
age = 25
...
# not operator.
# It converts truthy values to False and falsy values to True. Unlike the previous operators we looked at, not always returns True or False.
# Here are a few examples:
# Example Code
...
# Since is_admin is False, then not is_admin is saying not False which is True.
# So the message Access denied for non-administrators. will be printed.